...
int main(int argc, char* argv[]) {
  ...
  printf("%.16G %.16G %.16G\n", calInt(n, legRec, legCoef, legNod, func1),
         calInt(n, chRec, chCoef, chNod, func1), trapezium(n, -1, 1, func1));
  printf("%.16G %.16G\n", calInt(n, legRec, legCoef, legNod, func2),
         calInt(n, chRec, chCoef, chNod, func2));
  printf("%.16G %.16G %.16G\n", calInt(n, legRec, legCoef, legNod, func3),
         calInt(n, chRec, chCoef, chNod, func3), trapezium(n, -1, 1, func3));
  return 0;
}
...
double func1(double x) { return 1 / (1 + pow(x, 2)); }
...
double func3(double x) { return fabs(x); }
...
double calInt(int n, double met(), double coef[n], double nod[n],
              double func()) {
  double r = 0;
  if (met == legRec) {
    for (int i = 0; i < n; i++) {
